-- Security fixes for data leaks and access control
...
-- Public can only see id and role (for basic user info)
CREATE POLICY "Public can view basic profile info" ON public.profiles
  FOR SELECT TO anon
  USING (true)
  WITH CHECK (false); -- No inserts from anon
...
-- 6. Create secure views for public data
-- Safe public resumes view (no contact info)
...
-- Safe public vacancies view (no employer contact)
CREATE OR REPLACE VIEW public.vacancies_public_safe AS
SELECT 
  id,
  title,
  description,
...
-- 9. Create function to get current user role securely
CREATE OR REPLACE FUNCTION public.get_current_user_role()
RETURNS TEXT AS $$
  SELECT role::TEXT FROM public.profiles WHERE id = auth.uid();
$$ LANGUAGE SQL SECURITY DEFINER STABLE SET search_path = public;

-- 10. Add default deny policies where needed
-- Ensure all tables have explicit RLS enabled
ALTER TABLE public.events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.outbox_webhooks ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.data_access_log ENABLE ROW LEVEL SECURITY;

-- Add comments for documentation
COMMENT ON TABLE public.security_logs IS 'Audit trail for security events and access attempts';
COMMENT ON FUNCTION public.validate_password_strength IS 'Validates password strength and checks against common passwords';
COMMENT ON FUNCTION public.log_security_event IS 'Logs security events for audit purposes';
COMMENT ON VIEW public.resumes_public_safe IS 'Public view of resumes without contact information';
COMMENT ON VIEW public.vacancies_public_safe IS 'Public view of vacancies without employer contact information';